...
│   │                     │ │                     │               │
│   │  Your pods run here │ │  Your pods run here │               │
│   └─────────────────────┘ └─────────────────────┘               │
│                                                                 │
│   3 Podman containers = 3 Kubernetes nodes = 1 cluster          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```text
~/k8s-playground/
├── kind-config.yaml        # Cluster configuration (Chapter 0)
├── pods/                   # Pod manifests (Chapter 1)
├── deployments/            # Deployment manifests (Chapter 2)
├── services/               # Service manifests (Chapter 3)
└── ...
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   You ran: kubectl run my-first-pod --image=nginx               │
│                                                                 │
│   1. kubectl sent request to API Server                         │
│                              │                                  │
│   2. API Server stored pod   │                                  │
│      spec in etcd            ▼                                  │
│                        ┌──────────┐                             │
│   3. Scheduler saw     │ Scheduler│                             │
│      unscheduled pod   └─────┬────┘                             │
│                              │                                  │
│   4. Scheduler picked        │ "Put on worker node 1"           │
│      best node               ▼                                  │
│                        ┌──────────┐                             │
│   5. kubelet on that   │ kubelet  │                             │
│      node saw the pod  └─────┬────┘                             │
│                              │                                  │
│   6. kubelet told            │                                  │
│      container runtime       ▼                                  │
...
│   ✓ Saw the Control Plane components running as pods            │
│   ✓ Deployed your first container (nginx)                       │
│   ✓ Understood the flow from kubectl to running container       │
│                                                                 │
│   You now have a real Kubernetes environment to learn with!     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
